...
def parse_supercop_cycles(root: Path = Path("data/SUPERCOP")) -> pd.DataFrame:
    ...
    for p in files:
        ...
        rows.append(
            {
                "algorithm": alg,
                "supercop_keygen_cycles": find_one(["keypair", "keygen"]),
                "supercop_encaps_cycles": find_one(["encaps", "enc"]),
                "supercop_decaps_cycles": find_one(["decaps", "dec"]),
            }
        )
...
def main() -> None:
    ...
    def mk_nist_str(r):
        ...
        def f(x):
            if isinstance(x, float) and np.isnan(x):
                return "—"
            x = float(x)
            return str(int(x)) if x.is_integer() else f"{x:.0f}"
    ...
    nroot = find_nist_root()
    if nist.empty:
        print(f"NOTE: NIST KAT sizes not parsed (root={nroot}). Table will show liboqs sizes only.")
    if sup.empty:
        print("NOTE: SUPERCOP not parsed. Table will show only this work + sizes.")
